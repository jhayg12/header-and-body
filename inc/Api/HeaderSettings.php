...
class HeaderSettings extends TableController
{
    public $table_name;
    ...
    {
        switch( $column_name ) {
            case 'header-order-up':
            case 'header-order-down':
     ...
     **************************************************************************/
    function column_header_description( $item )
    {
        
        //Build row actions
        $actions = array(
            'edit'      => sprintf( '<a href="'. get_site_url() .'/wp-admin/options-general.php?page=edit_header&action=%s&hid=%s">Edit</a>','edit',$item['id'] ),
            'view'    => sprintf( '<a href="'. get_site_url() .'/wp-admin/options-general.php?page=view_header&action=%s&hid=%s">View</a>','view',$item['id'] ),
            'delete'    => sprintf( '<a class="delete-header" id="'. $item['id'] .'" href="'. get_site_url() .'/wp-admin/options-general.php?page=delete_header&action=%s&hid=%s">Delete</a>','delete',$item['id'] ),
        );
    ...
    {
        return sprintf(
            '<input type="checkbox" name="%1$s[]" value="%2$s" class="header-check"/>',
    ...
    function get_sortable_columns()
    {
        $sortable_columns = array(
            'header_description'     => array('header_description',false),     //true means it's already sorted
            'header-content'    => array('header-content',false),
            'header-status'  => array('header-status',false),
        );
    ...
    {
        $actions = array(
            'activate_header' => 'Activate',
            'deactivate_header' => 'Deactivate',
    ...
    function process_bulk_action() 
    {
        ...
        if( 'activate_header' === $this->current_action() ) {
            $header_data;
        ...
        if( 'deactivate_header' === $this->current_action() ) {
            $header_data;
        ...
        if ( isset( $_GET[ 'action' ] ) || ! empty( $_GET[ 'action' ] ) ) {
            ...
            if ( $_GET[ 'action' ] === 'head' ) {
                ...
                if ( isset( $_GET[ 'filter' ] ) || ! empty( $_GET[ 'filter' ] ) ) {
                    ...
                    $obj_data = $wpdb->get_results( "SELECT id, description, content, status, header_order, position FROM {$this->table_name} WHERE Status = '{$filter_by}' ORDER BY position ASC, header_order ASC" );
                    $obj_temp = json_decode( json_encode( $obj_data ), true );
                    for ( $x = 0; $x < count( $obj_temp ); $x++ ) {

                        $arr_temp = array(
                                    'id' => $obj_temp[$x]['id'],
                                    'header-order-up' => '<i title="Move Up" class="header-ord-up" id="'. $obj_temp[$x]['id'] .'">&#9650;</i>',
                                    'header-order-down' => '<i title="Move Down" class="header-ord-down" id="'. $obj_temp[$x]['id'] .'">&#9660;</i>',
                                    'header_description' => $obj_temp[$x]['description'],
                                    'header-content' => $obj_temp[$x]['content'],
                                    'header-status' => $obj_temp[$x]['status'],
                                );

                        // Position 0 is TOP and 1 is BOTTOM
                        if ( $obj_temp[$x]['position'] == 0 ) {
                            $arr_temp['header-position'] = '<label class="toggle-check">
                                                                    <input type="checkbox" id="'. $obj_temp[$x]['id'] .'" class="header-toggle-check-input" checked/>
                                                                    <span class="toggle-check-text"></span>
                                                                </label>';
                        } else {
                            $arr_temp['header-position'] = '<label class="toggle-check">
                                                                    <input type="checkbox" id="'. $obj_temp[$x]['id'] .'" class="header-toggle-check-input"/>
                                                                    <span class="toggle-check-text"></span>
                                                                </label>';
                        }
